require 'ostruct'
require 'test_helper'
require File.join(File.dirname(__FILE__), '..', 'lib', 'acl9', 'controller_extensions', 'dsl')
...
class DslTestAppliance
  ...
  def initialize
    ...
    @processor = Acl9::Dsl::Processor.new
  end

  def default_action
    @processor.default_action
  end

  def acl_block!(&block)
    @processor.acl_block!(&block)
  end

  def expression
    @processor.expression(Generator.new)
  end

  def permit(user, *args)
    allow?(user, *args).should == true

    self
  end

  def forbid(user, *args)
    allow?(user, *args).should == false
...
class DslTest < Test::Unit::TestCase
  ...
  def acl(&block)
    test_appliance = DslTestAppliance.new
    test_appliance.acl_block!(&block)
  ...
  describe "global roles" do
    ...
    it "#deny with several roles" do
      ...
      acl do
        ...
        deny :admin
        deny :cool
      end.show_code.forbid(@user).forbid(@user2).permit(nil).permit(@user3)
    end
  end
